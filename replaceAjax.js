var ajaxList = [];
function modifyResponse(method, url, response) {
  ...
  if (this.readyState === 4) {
    ...
    if (url && method) {
      ...
      ajaxList.push( url);
      ajaxList.push('\n');
      console.log('输出了~',ajaxList.length )
    }
  }
}
...
function reWriteOpen(originOpen) {
  ...
  return function (method, url) {
    this.addEventListener("readystatechange", curryFun(modifyResponse, this, method, url));
...
window.addEventListener("message", function (event) {
  console.log('replaceAjax event',event)
  if (event.source !== window || event.data !== 'saveFile') {
    return;
  }
  console.log(event.data, '我接受到了点击保存', ajaxList);
  if(!ajaxList.length) return; // 看下为什么会需要两次弹窗,  看起来 两个 origin都发过来origin了
  var file = new File([ajaxList.join('\n')], "api.txt", { type: "text/plain;charset=utf-8" });
  saveAs(file);
}, false);
...
//     console.log('request3456',request)
